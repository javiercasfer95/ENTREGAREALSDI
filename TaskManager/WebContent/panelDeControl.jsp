...
<html>
<head>
<link rel="stylesheet" href="css/tablaUsuarios.css" type="text/css" media="all"> 
			...
			<dl>
				...
				<tr id="tr_${entry.status}">
					<dd id="item_${i.index}">
						...
						<td>${entry.email}</td>
						<td>${entry.status}</td>
							<td id="td_${entry.status}"> 
							<a id="link_${entry.status}"
									href="cambiarEstado?id=${entry.id}"> ${entry.status=='ENABLED' ? 'DISABLE' : 'ENABLE'}</a>
									
							<!--  <form action="cambiarEstado?id =${enty.id}" method="post" name="opcionCambiarEstado" >
